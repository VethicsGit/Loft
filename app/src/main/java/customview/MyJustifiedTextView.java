...
public class MyJustifiedTextView extends JustifiedTextView {
    ...
    public MyJustifiedTextView(final @NonNull Context context, final AttributeSet attrs) {
        super(context, attrs);
        if (!isInEditMode()) {
            final SupportActivity activity = (SupportActivity) context;
    ...
    private int getLineAtCoordinate(final @NonNull Layout layout, final float y) {
    ...
    private int getOffsetAtCoordinate(final @NonNull Layout layout, final int line, final float x) {
